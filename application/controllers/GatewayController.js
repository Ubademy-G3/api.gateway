...
exports.signup = async (req, res) => {
  try {
    ...
    const user = await axios.post(`${process.env.USERS_SERVICE_URL}/users`, req.body, { headers: {'Authorization': process.env.USERS_APIKEY} });
    res.status(200).json(authUser.data);
  } catch (err) {
...
exports.login = async (req, res) => {
  try {
    ...
    res.status(200).json(serializer(loggedUser));
  } catch (err) {
    console.log(err)
    if (err.response && err.response.status && err.response.data) {
      return res.status(err.response.status).json(err.response.data);
